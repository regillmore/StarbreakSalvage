...
The measured Phase 9 baseline run is roughly six minutes. Phase 10 is now underway around expedition depth and shipcraft: work orders 091-096 add a deterministic seed-plus-save expedition graph, multi-stage mission state machine and objective anthology, modular frames and hardpoints, a run-local salvage foundry with weapon evolution, and three reusable capital-ship/station/wreck-convoy set pieces. The Hecaton Ledger Ark, Bloom Spindle Exchange, and Court Wreck-Train Crown use dependency-gated subsystems, fixed-world safe lanes, bounded turret/hangar pressure, staged rewards, objective hooks, and a finale boss lock; debug runs can jump to the current assembly with `U`. Later work orders cover run-local faction rivals, crew/wingmates, and a local Scenario Lab.
