...
Add `?debug=1` to the local or deployed URL to show the debug overlay with FPS, scene, seed, total entity count, enemy count, projectile split, pickup/effect count, telegraph count, distance, scroll speed, arena phase when active, scenario label, active input mode, selected contract theme, background primitive/layer count, active landmark/hazard count, viewport class, presentation scale, gameplay safe-frame size, and fixed combat world size during gameplay.
