...
The main menu seed field accepts blank/default, `random`, known labels such as `LASER-TAX-404`, or any custom label. Blank/default seeds use `STARBREAK-SMOKE`; `random` resolves to a copyable generated seed when the run starts. You can also add `?seed=LASER-TAX-404` or another seed label to the URL to preview deterministic contract, route, objective, sector length, wave distance marks, reward, shop, background, landmark, hazard, boss arena, and boss generation. Run summaries include route history, items, unlock reasons, win/loss detail, sector distance reached, and a copy-ready seed link for sharing the same generated contracts, route, rewards, shop inventory, background plans, feature plans, arena marks, wave marks, and boss schedule.
