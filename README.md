...
The measured Phase 9 baseline run is roughly six minutes. Phase 10 is now underway around expedition depth and shipcraft: work orders 091-097 add a deterministic seed-plus-save expedition graph, multi-stage mission state machine and objective anthology, modular frames and hardpoints, a run-local salvage foundry with weapon evolution, three reusable capital-ship/station/wreck-convoy set pieces, and run-local faction campaigns with recurring rival captains. The Hecaton Ledger Ark, Bloom Spindle Exchange, and Court Wreck-Train Crown use dependency-gated subsystems, fixed-world safe lanes, bounded turret/hangar pressure, staged rewards, objective hooks, and a finale boss lock; debug runs can jump to the current assembly with `U`. Faction aid, theft, mercy, contracts, mission outcomes, and rival escape/capture/destruction now change later briefings, combat pressure, routes, shops, crew-offer signals, set-piece ownership, and finale conditions. Debug runs can stage a returning-rival campaign with `R`. Later work orders cover crew/wingmates and a local Scenario Lab.
