...
The measured Phase 9 baseline run is roughly six minutes. Phase 10 is now underway around expedition depth and shipcraft: work order 091 adds a deterministic seed-plus-save expedition graph with stable acts, sector plans, mission legs, encounter nodes, optional branches, checkpoint/finale gates, public HUD/summary/debug path readouts, and v4-to-v5 save migration. Its generated content budget targets about 16.0 minutes on required nodes and 19.7 minutes with every optional node; live play still uses the existing one-operation-per-sector compatibility path until work order 092 adds the mission state machine. Later work orders cover modular frames and hardpoints, salvage engineering and weapon evolution, capital-ship/station set pieces, run-local faction rivals, crew/wingmates, and a local Scenario Lab.
