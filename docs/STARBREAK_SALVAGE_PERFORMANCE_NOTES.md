...
Work order 120 replaces one beam rectangle with one distant immutable world vector plus derived arena-clipped track and finite bolt segments. Each projection uses four bounded clipping comparisons; actor collision performs one point-to-segment test per live enemy, boss, and active ally only while the clipped visible bolt exists. World/set-piece damage uses at most 24 continuous expanded bolt boxes and existing per-target hazard cooldowns; combat actors use a transient keyed cooldown map. Rendering is capped at one faint track stroke, four layered bolt strokes, one leading flare, one entrance marker, one exit reticle, and four track marks. Reduced motion uses two marks, while performance mode removes marks and shadow blur. The moving bolt is arithmetic over the existing hazard phase, not a particle, projectile, beam entity, RNG stream, or save field.

The refined work order 120 release build emits 857.62 kB minified/232.58 kB gzip initial JavaScript and unchanged 31.96 kB CSS/7.19 kB gzip. This is a 2.15 kB minified/0.69 kB gzip increase over the initially deployed work order 120 beam and an 8.78 kB minified/2.74 kB gzip increase over work order 119. No warning threshold changed; the existing initial-chunk warning remains open.
...
The command deck is a lazy production chunk and carries no bitmap assets. Carrier staging does not change the 1,689/2,199-second graph projections because the command action is optional and uses native DOM interaction rather than an inflated combat schedule. Snapshot size remains capped at 512 KiB and endurance reports carrier history/cargo maxima alongside existing engineering/faction/crew/timeline bounds.
...
Work order 110 adds no combat actor, projectile, effect, collision, targeting, or proc path. Three Scenario Lab fixtures reuse existing carrier, frontier, snapshot, and DOM scenes. The duration audit generates two immutable run skeletons only when its debug-only card opens, folds at most 105 nodes per profile, and labels its six measurements as structural projections rather than frame-time or stopwatch telemetry. Endurance remains bounded to 32 cycles, seventeen snapshot boundaries per cycle, the 512 KiB snapshot cap, and existing domain history ceilings.
