...
The source and target use the same orange `choice` styling and one compositor-only pulse per actionable sector node. The 1280×720 Chromium contract asserts that the three-route body has no internal overflow. The post-optional and recovery presentations still render exactly three cards, while reduced-motion and performance modes retain the existing static-ring fallback.

The release build emits 917.20 kB minified/249.89 kB gzip initial JavaScript and 64.46/13.30 kB CSS. Relative to work order 137, the shared commitment seam, node-owned presentation, direct-entry handoff, and orange ready styling add 1.07/0.27 kB JavaScript and 0.41/0.06 kB CSS. No dependency, simulation budget, snapshot, RNG stream, or warning threshold changes; the existing initial-chunk warning remains open.
