...
Work order 120 replaces one beam rectangle with one immutable world track plus one derived finite bolt segment. Actor collision performs one point-to-segment test per live enemy, boss, and active ally while the visible bolt is damaging. World/set-piece damage uses at most 24 continuous expanded bolt boxes and existing per-target hazard cooldowns; combat actors use a transient keyed cooldown map. Rendering is capped at one faint track stroke, four layered bolt strokes, one leading flare, one source emitter, one endpoint reticle, and four track marks. Reduced motion uses two marks, while performance mode removes marks and shadow blur. The moving bolt is arithmetic over the existing hazard phase, not a particle, projectile, beam entity, RNG stream, or save field.

The refined work order 120 release build emits 856.92 kB minified/232.28 kB gzip initial JavaScript and unchanged 31.96 kB CSS/7.19 kB gzip. This is a 1.45 kB minified/0.39 kB gzip increase over the initially deployed work order 120 beam and a 8.08 kB minified/2.44 kB gzip increase over work order 119. No warning threshold changed; the existing initial-chunk warning remains open.
