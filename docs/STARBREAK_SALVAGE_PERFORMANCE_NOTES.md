...
Each routine meteor storm derives one rounded travel pocket and exactly ten deterministic impact timelines from the existing hazard record. At any instant, simulation retains only the impacts in their bounded visible phases and exposes collision rectangles only during the brief impact phase. Exact circle checks replace broad-lane damage while reusing the existing bounded player, enemy, boss, ally, set-piece, and environment scans plus hazard cooldown ledger. No projectile, actor, pickup, timer, retained history, save field, or RNG draw is added.

Canvas rendering remains in the established under-bullet hazard layer. One pocket, a bounded dotted route, up to three forecast circles, and the currently visible telegraph/impact/afterglow circles replace the squall's full-height lanes and wreckage field. Performance mode reduces track and fragment ornament; reduced motion removes trail drift without changing pocket travel, impact timing, or collision. `salvage_squall` remains a content definition and pure model for future authored encounters but is absent from routine choice arrays and therefore adds no live encounter density.

The release build emits `1,007.25 kB` minified / `276.03 kB` gzip initial JavaScript and unchanged `95.61 kB` / `19.25 kB` CSS, increases of `7.40 kB` / `1.95 kB` JavaScript over work order 186. The existing 500 kB chunk advisory remains; no dependency, simulation cap, save/snapshot schema, RNG stream, or warning threshold changed.
