...
- Player ship appearance rendering remains shape-based canvas work: one selected silhouette path, simple mount primitives, appearance-derived colors, high-contrast substitutions, and a fixed hit-radius ring. New Game keeps three static inline-SVG comparison silhouettes and adds one highlighted live-fire DOM preview derived from the same appearance and bounded production-volley model. The animation is selection-screen-only, creates at most 48 projectile spans, disables motion in reduced/performance modes, and never enters gameplay simulation or canvas work.
...
Work order 110 adds no combat actor, projectile, effect, collision, targeting, or proc path. Three Scenario Lab fixtures reuse existing carrier, frontier, snapshot, and DOM scenes. The duration audit generates two immutable run skeletons only when its debug-only card opens; work order 139 narrows each profile fold to at most 60 fresh executable nodes while retaining compatibility records outside the measurement. Its six results remain structural projections rather than frame-time or stopwatch telemetry. Endurance remains bounded to 32 cycles, seventeen snapshot boundaries per cycle, the 512 KiB snapshot cap, and existing domain history ceilings.
...
The post-sector scene reuses the same five-sector constellation and never mounts both action sets at once: selecting the source renders one optional button, while selecting the target renders the three already-generated route buttons. Route selection performs one existing default-branch dispatch and, when required, one existing relief dispatch before entering the unchanged route pipeline. After same-act route settlement, the existing briefing and entry events execute synchronously instead of mounting a second constellation scene. These operations replace user-mediated scene transitions; they add no generation fold, RNG draw, snapshot field, timer, frame update, or gameplay hot-loop work.

The source and target use the same established beige `choice` styling and one compositor-only pulse per actionable sector node. Selection strengthens that beige focus/glow without introducing a second ready-state hue. The 1280×720 Chromium contract asserts that the three-route body has no internal overflow. The post-optional and recovery presentations still render exactly three cards, while reduced-motion and performance modes retain the existing static-ring fallback.

The release build emits 917.19 kB minified/249.90 kB gzip initial JavaScript and 64.46/13.28 kB CSS. Relative to work order 137, the shared commitment seam, node-owned presentation, direct-entry handoff, and unified beige ready styling add 1.06/0.28 kB JavaScript and 0.41/0.04 kB CSS. No dependency, simulation budget, snapshot, RNG stream, or warning threshold changes; the existing initial-chunk warning remains open.

## Work order 139 constellation-aligned operation budgets

Fresh sector entry now creates one full-profile required `GameplayScene` instead of a shortened advance scene, a staging DOM scene, and a second required gate scene. The optional pursuit still creates at most one separate gameplay instance after the first instance has settled through the existing zero-retained-world cleanup boundary. Stable compatibility stage records add no actors, projectiles, effects, hooks, timers, or DOM to a fresh voyage; they are consulted only when restoring an older v11 target or opening a dedicated Scenario Lab fixture.

The mission reducer performs fewer fresh transitions and option projection performs fewer availability folds. A settled pursuit's existing one-record influence lookup now targets the following gate rather than the retired fresh advance; its constant-breadth cost and 1.12 scroll/1.20 wave bounds are unchanged. Combat itself retains the existing full-sector scroll, wave, hazard, environment, boss, set-piece, actor, projectile, effect, ally, and proc ceilings. No per-frame condition, RNG draw, secondary simulation, or new retained history was introduced.

The release build emits 916.91 kB minified/249.76 kB gzip initial JavaScript and 64.46/13.28 kB CSS. Relative to work order 138, simplifying the live schedule and teaching the capacity audit about compatibility nodes reduces initial JavaScript by 0.28/0.14 kB; the stable definitions remain data in the existing graph. Executable-node projection now reports 1,009 seconds (16.82 minutes) standard, 665 seconds (11.08 minutes) at Act II extraction, and 1,264 seconds (21.07 minutes) with all paired holds. These are authored estimates, not a deployed stopwatch; a completionist measurement remains required. No dependency, simulation cap, snapshot version, RNG stream, or warning threshold changed.

## Work order 140 immediate-reward budgets

Required gate completion now mounts the existing `RewardScene` before the constellation rather than after an onward route event. Reward generation adds one bounded reverse route-history lookup and reuses the bounded objective-history modifier fold; neither runs in the gameplay fixed-step loop. Item/credit selection then mounts the same constellation that previously appeared first. Optional completion and route settlement each remove a later reward-scene mount, so a completionist path has one reward presentation per sector instead of a possible redundant stop.
...
## Work order 142 constellation-suspension budgets

Suspension is a scene-boundary operation: one existing snapshot encode/storage write occurs on explicit button or Escape input, followed by a main-menu mount. It adds no fixed-step update, actor query, collision, projectile, effect, renderer, timer, generation fold, or RNG draw. Resume already regenerates the run and reprojects the checkpoint target; no DOM, canvas, or live combat state is retained.

The shared footer adds one button only while a suspend callback exists and wraps under the existing narrow navigation breakpoint. The release build emits 918.80 kB minified/250.23 kB gzip initial JavaScript and 64.68/13.33 kB CSS. Relative to work order 141, the checkpoint-success guard, three callback bindings, presentation, and browser regression add 1.02/0.23 kB JavaScript and 0.22/0.05 kB CSS. No dependency, simulation cap, generated plan, RNG stream, permanent-save version, snapshot version, or warning threshold changed.

## Work order 146 destination-route-effect budgets

Route-effect selection evaluates exactly the target node's three existing candidates when a route read model is projected. It computes one minimum, one maximum, three normalized risk positions, and one weighted seeded draw outside the gameplay fixed-step loop. The target-sector index and id isolate the seed stream per node; source history, frame state, and snapshot presentation do not participate. Shared nodes therefore regenerate one effect from every parent and after resume without adding persisted state.

The route detail replaces three interactive cards with one bounded effect dossier and one commit button. Each ready node adds only the selected effect label to its existing state string; no canvas actor, projectile, effect, timer, collision, targeting, hook, or scene-update work is introduced. The release build emits 932.12 kB minified/254.39 kB gzip initial JavaScript and 68.09/13.93 kB CSS, increases of 1.00/0.38 kB JavaScript and 0.25/0.03 kB CSS over work order 145. The existing 500 kB chunk notice remains; no dependency, combat budget, route-outcome schema, save/snapshot version, or warning threshold changed.

## Work order 152 contract-board preview budgets

Contract selection resolves exactly three bounded presentation models when the scene opens. Each model creates the existing small engineering projection, fits one deterministic starter item, and resolves at most six representative volleys under the established proc budget; no candidate model advances RNG or enters a frame update. Only the selected contract mounts live projectiles. The other cards retain static ship SVGs, and replacing the selection rebuilds at most 48 projectile spans plus one bounded dossier.

Reduced-motion and performance modes keep their existing static representative positions and omit echo volleys in performance mode. Candidate ignition, metric, and optional Contract Survey Rig blocks exist only in the New Game DOM and create no gameplay actor, projectile, canvas, collision, snapshot, or retained-history work. The release build emits 931.94 kB minified/254.25 kB gzip initial JavaScript and 79.56/15.90 kB CSS, increases of 4.01/1.13 kB JavaScript and 6.61/1.06 kB CSS over work order 151. The existing 500 kB chunk notice remains; no dependency, combat budget, RNG stream, save/snapshot version, or warning threshold changed.

## Work order 153 direct-victory budgets
...
## Work order 187 meteor-pocket budgets

Each routine meteor storm derives one rounded world pocket and exactly twelve deterministic impact timelines from the existing hazard record. At any instant, simulation retains only the impacts in their bounded visible phases and exposes collision rectangles only during the brief impact phase. Exact circle checks replace broad-lane damage while reusing the existing bounded player, enemy, boss, ally, set-piece, and environment scans plus hazard cooldown ledger. World placement reads the existing actual scroll distance when runtime pause compensation is active. No projectile, actor, pickup, timer, retained history, save field, or RNG draw is added.
...
## Work order 190 primary-weapon circuit budgets

Circuit projection now reads at most the one mounted primary weapon instead of folding capacity across every mount. The resulting ordered pool remains capped at six slots, and replacement uses the existing bounded assignment reconciler. Capacity is derived from immutable component slot, source, sector, and quality fields; no save field, migration, generation pass, RNG draw, projectile, actor, effect, collision, timer, or fixed-step work is added.

Foundry renders at most six rail cards and one primary extension chip. The sixth component metric exists only on primary installed and cargo cards, so non-primary projections produce fewer DOM cells than before. Grid Envelope replaces its previous aggregate label with the same primary capacity authority. The release build emits `1,010.46 kB` minified / `276.92 kB` gzip initial JavaScript and `95.68 kB` / `19.25 kB` CSS, increases of `0.45 kB` / `0.17 kB` JavaScript and `0.07 kB` / effectively unchanged gzip CSS over work order 189. The existing Vite large-chunk advisory remains; no dependency, simulation cap, save/snapshot schema, route topology, RNG stream, static base path, or warning threshold changed.

## Work order 191 persistent-hull and dock-repair budgets

Hull carry adds one scalar copy at sector advance and one existing carry-policy lookup when combat projection is created. Effective current/max hull is derived only at scene boundaries and repair clicks; it does not enter the gameplay fixed-step loop beyond supplying the already-supported `startingHull` value. Repair performs one bounded credit spend, checkpoint replacement, and timeline append without generating inventory or advancing RNG.

The shop adds one fixed service row and at most one visual segment per effective hull point when the DOM scene mounts or redraws after a purchase. Navigation replaces an existing resource value and adds no node, route, timer, retained history, or animation. The release build emits `1,013.73 kB` minified / `277.83 kB` gzip initial JavaScript and `97.37 kB` / `19.56 kB` CSS, increases of `3.27 kB` / `0.91 kB` JavaScript and `1.69 kB` / `0.31 kB` CSS over work order 190. The existing Vite large-chunk advisory remains; no dependency, actor/projectile/effect cap, save/snapshot schema, route topology, RNG stream, static base path, or warning threshold changed.

## Work order 192 wing-rendezvous departure budgets

The departure model captures at most the existing four active allies and eight drones once when sector settlement hands off. Each frame performs one bounded twelve-entry pose projection and one small render lookup; it does not update follower AI, acquire targets, run collision, advance combat actors, or draw from RNG. Solo departures allocate an empty manifest and keep their original phase thresholds and duration.
...
## Work order 194 bounded reward-manifest budgets

Reward generation still performs one bounded weighted selection from the existing named stream. It now requests three circuit items, or four when one context-specific permanent option unlock applies, instead of folding run-local modifiers into an expanding count. Removed count paths retain only constant-size bias/economy projections. The compatible primary weapon and credit fallback remain separate, so reward DOM volume is bounded at five ordinary or six upgraded cards rather than the observed nine-plus late-run cards.

The two-row desktop treatment changes only reward-scene CSS and semantic count attributes; it adds no animation, retained history, actor, projectile, effect, timer, fixed-step branch, or RNG draw. The checked build emits `1,021.90 kB` minified / `280.30 kB` gzip initial JavaScript and `100.24 kB` / `20.07 kB` CSS, changes of `-0.25 kB` / `-0.02 kB` JavaScript and `+0.52 kB` / `+0.13 kB` CSS over work order 193. The existing Vite large-chunk advisory remains; no dependency, save/snapshot schema, route topology, static base path, or warning threshold changed.

## Work order 195 navigation header utility budgets
...
## Work order 199 contract-board weapon-glyph budgets
