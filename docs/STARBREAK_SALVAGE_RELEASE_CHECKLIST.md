...
- The current main JavaScript bundle is 914.17 kB minified (248.88 kB gzip) after work order 135 removes the redundant operational-map presentation from the fresh post-sector path. Several integration modules remain large: `CombatState`, `CanvasRenderer`, `GameApp`, `GameplayScene`, and content validation. These are next-phase scaling warnings, not hidden release exceptions.
