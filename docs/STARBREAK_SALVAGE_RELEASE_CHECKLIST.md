...
- The current main JavaScript bundle is 916.14 kB minified (249.61 kB gzip) after work order 136 embeds compact route plotting into the constellation and removes the standalone route-scene importer. Several integration modules remain large: `CombatState`, `CanvasRenderer`, `GameApp`, `GameplayScene`, and content validation. These are next-phase scaling warnings, not hidden release exceptions.
