...
Work order 141 removes the stale Act I constellation and Act II route decision after sector 5. The operation reward now hands through the authored default branch, relief, and extraction directly into the midpoint refit, without generating a route outcome, shop/event, or component.

Work order 142 lets every live constellation safely suspend to the main menu by button or Escape. A successful snapshot write precedes the exit, and Resume reconstructs the same briefing, flat post-sector board, or unresolved route plot without replaying settlement.
...
| Performance                | Pass with warning     | Combined stress caps and debug counts are explicit. Functional Chromium smoke passes. The 918.80 kB minified main bundle still exceeds Vite's 500 kB warning threshold; sustained frame-time/allocation profiling and further code splitting remain next-phase work. |
...
| Safe checkpoint UX    | Pass              | Briefing, operation-entry, and settled post-sector/route writes are automatic; constellation button/Escape suspension and main-menu resume/discard work by keyboard and pointer and explain the boundary. |
...
| Loading boundary      | Pass with warning | Initial JavaScript is 918.80 kB minified/250.23 kB gzip; low-frequency audit and service scenes remain lazy and the existing warning remains active.                   |
...
- The current main JavaScript bundle is 918.80 kB minified (250.23 kB gzip) after work order 142 adds safe constellation suspension and exact checkpoint reconstruction. CSS is 64.68 kB minified/13.33 kB gzip. Several integration modules remain large: `CombatState`, `CanvasRenderer`, `GameApp`, `GameplayScene`, and content validation. These are next-phase scaling warnings, not hidden release exceptions.
