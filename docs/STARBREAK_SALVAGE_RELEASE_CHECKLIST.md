...
| Unit and deterministic tests  | Pass            | `npm run check` - 41 files, 233 tests                                                    |
...
| Debug/performance tools    | Pass   | `?debug=1` supports five boss shortcuts, dense combat stress, forced player destruction with destruction-progress telemetry, forced sector completion with exit-progress telemetry, quiet long-scroll traversal, forced summary, banked scrap/upgrade readiness, run resources, sector-plan telemetry, granular entity/projectile/pickup/effect/feature counters, distance/speed/arena counters, input mode, HUD mode, contract theme, background primitive/layer count, viewport scale/DPR, canvas pixel size, safe-frame origin/size, and fixed world size. |
...
| Phase 5 readiness     | Pass   | Phase 5 now has banked scrap spending, upgrade bay icons, sector exit/toast feedback, lunar surface content, richer ship destruction, and automated deterministic smoke/debug instrumentation implemented as first passes; release hardening remains next. |

## Manual Browser Smoke

| Browser                 | Load    | Start Run | Combat  | Mouse   | Pause   | Settings | Summary | Narrow View | Debug Perf | Long Scroll | Notes                                                                                 |
| ----------------------- | ------- | --------- | ------- | ------- | ------- | -------- | ------- | ----------- | ---------- | ----------- | ------------------------------------------------------------------------------------- |
| Chromium via Playwright | Pass    | Pass      | Pass    | Pass    | Pass    | Pass     | Pass    | Pass        | Pass       | Pass        | `npm run test:e2e` passes with Codex escalation and covers preview/HUD/debug metrics, Upgrade Bay, exit, destruction, and Lunar Surface smoke. |
...
- The debug overlay reports granular entity, scroll/background/feature, input mode, HUD mode, contract theme, progression, run resources, sector plan, viewport/DPR/canvas, safe-frame, and fixed-world counts; frame-time sampling and allocation timing remain future instrumentation work.
...
- Lunar Surface now has deterministic background, landmark, hazard, encounter-pacing content, and Chromium traversal smoke; manual smoke should still check bullet readability over dust plumes, mining lasers, and surface-defense arcs. Player destruction now has a first-pass bounded debris/cockpit/transponder beat plus Chromium smoke, but balance and manual readability checks remain useful.
