...
Work orders 051-056 baseline, refreshed through work order 225. This document records the active item catalog after the Noita-style circuit pivot retired Boss Pressure from live rotation, moved eleven passive items into permanent progression, added six exclusive apex spoils, refilled the retired active slots, converted Prototype Vent Script into an ordered stored-heat modifier, made arc a projectile-carried secondary discharge, and consolidated the heat/prototype family around one shared thermal resource. The source of truth remains `src/content/items.ts`; repeatable coverage checks live in `src/content/itemCatalogAudit.ts` and `tests/unit/itemCatalogAudit.test.ts`.
...
| ------------------------------ | ---------- | ---------------------------------------------------------------------------------------- |
| `onFire`                       | 32         | Volley shaping, drones, split shots, missiles, thermal routing, and ordered cadence.     |
...
| Heat/Prototype   | 7     | Sources, thermostats, sinks, and stored-heat projectile payoffs now share one ordered thermal signal.         |
...
- Plasma Seed Crucible is an explicit source, injecting 8% of weapon heat capacity before later ordered stages resolve while retaining its trait-to-plasma projectile conversion.
- Overheat Oracle is a five-volley thermostat: it banks 18% capacity while cool, then spends 18% while hot to produce an amplified plasma phase omen.
- Heat Sink Saint remains the passive efficiency anchor and becomes an active hot-band sink, spending 6% capacity to stabilize stronger, longer-lived heat shots.
- Heat Signature Loop is the payoff. Heat/plasma projectiles scale from +5% to +30% impact and gain size and reach according to the shared stored-heat ratio at spawn time.
- Prototype Vent Script remains the large 32% spender and can consume heat generated by an earlier source in the same volley, preserving circuit-order agency.
- Combat and Hardpoint Control use the same cold, working, hot, critical, and overheated bands. The Foundry simulation reports generated/spent circuit flow, and combat presents distinct intake/sink cues without adding save state or RNG.
