...
- One shared pure geometry model derives a fixed-X world pocket, twelve deterministic impact schedules, visible phase, concise warning text, and exact circular damage areas from the authored hazard record.
- The pocket itself never damages. Only impact-phase circles do, and each circle is small enough to leave navigable gaps inside the storm when several local telegraphs overlap.
- Telegraph, impact, and afterglow phases are visually distinct; contracting rings, crosshairs, impact streaks, and the text readout communicate timing without relying on color.
...
- The storm is authored with a long 600-640 unit active span, spawns completely above the camera, and scrolls through the arena at its seeded world X already cycling impacts.
- No route line, direction arrow, whole-pocket forecast, or pre-entry HUD warning reveals the storm path; the cloud and its local impact telegraphs enter together.
- Actual scroll distance controls pocket position during runtime pause compensation, so impact cycles may resolve while a locked arena cannot make the world-anchored cloud drift.
- Reduced-motion, performance, and high-contrast settings preserve timing and collision while bounding fragment ornament.
...
Status: implemented and refined after deployment. `MeteorStorm` owns world-scroll pocket placement, deterministic impact placement and timing, circular damage geometry, warning copy, and the current-distance-aware browser fixture. `HazardZoneBehavior`, `SectorHazards`, `GameplayScene`, and `CanvasRenderer` consume that shared result. The broad ellipse is presentation only; simulation exposes only its momentary impact circles. Feature and condition generation reserve the complete long-lived span instead of truncating late-slot storms near sector exit.
