...
- `ExpeditionEndurance` runs every current Scenario Lab definition plus a final-sector checkpoint through repeated snapshot create/export/restore cycles, checking deterministic reports, pending-foundry cleanup, 512 KiB snapshot limits, 96-entry timeline and crew-arc history, 64-entry faction/crew histories, and set-piece/item/engineering presence.
...
- Production build smoke covers the initial JavaScript/CSS plus lazy Scenario Lab, timeline, command-deck, and Crew Quarters chunks through emitted HTML/module loading; manual storage quota, browser eviction, multi-tab contention, and long real-device restore timing remain open.
