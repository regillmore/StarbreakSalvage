...
- item card view models for reward, shop, vault, archive, and summary surfaces, including high-contrast and keyboard focus state; work order 058 adds shared model coverage and E2E checks, work order 205 protects hook-derived triggers, non-live compatibility badges, stable action/price footers, and the one-page reward manifest, and work order 206 pins the three-slot base market, additive stock bonuses, stable depletion, and three-column shop readability;
...
- work order 203 adds Crater Shadow permanent/restored precedence, Lunar/ordinary charge values, non-generative upgrade fingerprints, Penumbra projected-centerline selection, reverse-order neutrality, exact phase/plasma geometry changes, and bounded stress/catalog regressions;
...
- work order 186 pins the environment-stress shortcut to one deterministic salvage squall and verifies its three-surge calm-channel sequence, active-versus-lull collision geometry, both seeded travel directions, player/enemy/boss/ally damage policy, concise warning states, and browser-visible `SURGE/LULL n/3 | CALM ...` readout;
- work order 187 repoints that shortcut at a current-distance-aware meteor fixture and verifies offscreen active spawn, fixed-X world-scroll motion, twelve-impact scatter, hidden pre-entry presentation, telegraph/impact/afterglow timing, pause-stable position, exact circular player/enemy/boss/ally/environment damage, natural cleanup, and browser-visible `MARKED` / `IMPACTING` states without a route line; the work order 186 squall remains special-only and outside routine schedules;
- work order 188 verifies Mining Laser Transit's permanent/restored reward-bias equivalence and no-double-application boundary across fixed seeds, the 15-card Upgrade Bay and Archive prerequisite copy, Strata-Bore Collimator's plasma-first versus collimator-first circuit order, capped trait scaling, beam-laser live-fire presentation, catalog breadth, source-weighted snapshots, and responsive managed-browser layout;
- work order 189 verifies Ambush Insurance Stamp's permanent/restored salvage-and-bias equivalence and no-double-application boundary, the 16-card Upgrade Bay and Navigation prerequisite copy, Claimant Arc Seal's overkill-first versus seal-first order, shared standard-arc profile, unchanged primary impact, catalog breadth, source-weighted snapshots, and managed-browser layout;
- work order 190 verifies the exact primary-only capacity matrix (contract three, Act I two-to-five, Act II+ three-to-six), zero capacity and no `S` presentation on every non-primary kind, alternating universal conduit channels, deterministic tail reconciliation across primary replacement, starter ignition plus two open slots, signed primary comparison deltas, Grid Envelope parity, restored snapshots, and responsive managed-browser expansion from three to five slots;
- work order 191 verifies numeric checkpoint hull carry into the next operation, null-as-full initial compatibility, one-point clamped repair, Act I and later-act labor prices, full and critical shop states, insufficient-credit disabling, navigation current/max tone, finite-rack coexistence, resume/snapshot continuity, desktop and 390-by-700 layouts, and a clean managed-host lifecycle;
...
- Work order 179 protects the permanent Low-Orbit route refund, restored-item no-double-pay precedence, non-generative upgrade fingerprint, 11-card Upgrade Bay, 60-item active breadth, source-weighted snapshots, Gangue's upstream-only light-branch compression, plasma trait propagation, cumulative Hardpoint stage output, unchanged projectile count, and the bounded 29-item hook-storm profile.
- Work order 180 protects unchanged primary impact, deterministic nearest-distinct secondary targeting, standard/heavy arc profiles, no-target expiry, phase-retained charge, enemy/boss/ally accounting, capped endpoint effects, removal of kill-only arc damage, item-specific standard/heavy attachment, ordered Hardpoint arc deltas, and bounded Canvas/DOM electrical presentation.
- Work order 181 protects permanent Route Ledger cash-out, restored-item no-double-pay precedence, non-generative upgrade fingerprinting, the 12-card Upgrade Bay, 60-item active breadth, deterministic source-weighted snapshots, Forkline's upstream-only outer-branch selection, unchanged primary impact/projectile count, standard arc propagation, cumulative Hardpoint arc output, and the bounded 29-item hook-storm profile.
- Work order 182 protects Prototype Vent's shared earlier-periodic condition count, explicit met/unmet signal output, semantic DOM state, immediate two-way reorder updates, unchanged Phase cadence annotation behavior, ordinary circuit summaries, and simulation neutrality.
- Work order 183 protects permanent Market Echo Shop/Repair reward breadth and bias, restored-item no-double-apply precedence, non-generative upgrade fingerprinting, the 13-card Upgrade Bay, 60-item active breadth, deterministic source-weighted snapshots, Faraday's upstream-arc ordering, preserved standard/heavy charge, cumulative Hardpoint tags, and the bounded 29-item hook-storm profile.
- Work order 200 protects the permanent lunar Surface Beacon ping, restored-item no-double-apply precedence, non-generative upgrade fingerprinting, the 17-card Upgrade Bay, 60-item active breadth, unchanged weighted reward snapshots, Parallax's upstream-secondary-only phase and flight extension, reverse-order neutrality, explicit Hardpoint condition output, and unchanged projectile count.
- Work order 201 protects exact one-volley direct-damage arithmetic, hit-dependent arc exclusion, effective-cadence least-common-cycle measurement, Prototype Vent five-versus-four ordering, a mixed 20-volley cycle, reorder-sensitive DPS, draft deltas, an unchanged 48-projectile visual cap, explicit sample-boundary copy, and responsive Hardpoint placement.
- Work order 203 protects Crater Shadow's permanent/restored no-double-apply boundary, the 18-card Upgrade Bay, 60 active and 75 compatibility item definitions, source-weighted reward snapshots, Penumbra's upstream multi-shot requirement and deterministic centerline selection, unchanged body impact/projectile count, 92% velocity, +1 radius, +0.18 seconds flight, phase/plasma propagation, explicit Hardpoint condition output, and the bounded 29-item hook-storm profile.
- Work order 214 protects Relic Ash Compass's permanent/restored Vault-bias equivalence and no-double-apply boundary, its non-generative upgrade fingerprint, the 19-card Upgrade Bay, 66 active and 82 compatibility item definitions, unchanged 20-card Vault breadth, Ashwake's three-source cap and upstream-phase order, exact echo geometry/impact/lifetime/proc traits, explicit Hardpoint met/unmet output, and bounded hook dispatch.
