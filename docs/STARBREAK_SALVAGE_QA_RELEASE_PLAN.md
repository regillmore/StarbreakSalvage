...
- work order 187 repoints that shortcut at a current-distance-aware meteor fixture and verifies both travel directions, deterministic pocket motion and ten-impact scatter, forecast/telegraph/impact timing, exact circular player/enemy/boss/ally/environment damage, warning text, natural cleanup, and browser-visible `MARKED` / `IMPACTING` states; the work order 186 squall remains special-only and outside routine schedules;
