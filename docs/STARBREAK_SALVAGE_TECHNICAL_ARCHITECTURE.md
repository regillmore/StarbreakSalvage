...
### Work orders 132, 145, 159, 160, and 169 item circuit boundary

- `ItemSockets.ts` is the authority between owned inventory and active mechanics. It derives a bounded conduit pool only from installed `EngineeringSnapshot.mounts` and each component's progression-scaled contribution, reconciles assignments against that pool, and projects the ordered active `ItemInstance[]` consumed everywhere else. Module-native channel labels remain derived affinity metadata, not fitting gates.
...
- Periodic volley cadence is authored once in `ItemHooks`. Prototype Vent Script is an order-sensitive passive: only periodic `onFire` stages earlier in `circuitOrder` resolve at `base + 1`, and each completed shifted cycle appends one generic `heat`-tagged projectile before later circuit stages execute. Hardpoint presentation queries the same cadence profile rather than parsing item copy or duplicating timing rules; the attack simulation exercises real consecutive volley indices, while cumulative stage cards receive later passive context without executing unrelated later stages.
- Hardpoint Control owns a reversible circuit draft beside the engineering draft. The UI exposes one numbered ship-level rail and a simple rack instead of component-local placement selectors. Installed and cargo components project circuit capacity as the sixth standard `S` stat beside power, heat, mass, command, and instability; cargo scrap value remains on its action instead of occupying a duplicate stat cell. Grid Envelope aggregates the same authority as live stages over installed capacity, while rail extension chips retain the module-level capacity map. Item badges describe effect domains rather than eligibility, and the attack simulation recomputes from the same ordered live list after every edit.
