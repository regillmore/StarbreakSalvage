...
- Work order 136 removes `RouteScene` from the fresh runtime graph. `src/game/RouteNavigation.ts` projects a compact source-edge-target/mission/options read model, while `SectorTransitionScene` renders it as a route mode on the actionable constellation node. Route mutation remains in `GameApp`/`RunSession` against the completed sector. Work order 140 later moves reward settlement ahead of this plot while retaining shop/event/component/advance ordering. Snapshot v11 reuses the `operationalMap` target for extraction-stage route plots and reconstructs presentation from run/session state.
...
- Starting item loadouts are three unique deterministic rolls from the starter profile, weighted by contract item bias and filtered by unlock access. Do not prepend a universal projectile-shaping kit: doing so erases weapon/ship silhouettes and causes downstream reward pools to exclude the same items for every contract.
...
### Work orders 132 and 145 item circuit boundary

- `ItemSockets.ts` is the authority between owned inventory and active mechanics. It derives a bounded conduit pool only from installed `EngineeringSnapshot.mounts` and each module's data-defined native/flex contribution, reconciles assignments against that pool, and projects the ordered active `ItemInstance[]` consumed everywhere else.
- Acquisition order remains ownership identity. `ItemSocketAssignment.componentId/socketIndex` is internal physical routing, while `circuitOrder` is an explicit independent processing order. Append allocates a compatible open conduit and places the item at the tail; earlier/later operations swap only logical order; eject preserves ownership in the inactive rack.
- Reconciliation first preserves a fully valid stored physical route. If engineering removes or replaces a source, a deterministic bounded augmenting-path match reroutes live items across the remaining compatible conduits in circuit order. Deliberately racked items stay racked, and a later item is dropped only when no legal maximum route remains.
...
- Hardpoint Control owns a reversible circuit draft beside the engineering draft. The UI exposes one numbered ship-level rail and a simple rack instead of component-local placement selectors. Component cards and rail extension chips report capacity/channel contributions; the existing attack simulation recomputes from the same ordered live list after every edit.
- Snapshot v12 retains the existing assignment shape and validates acquisition identity, socket shape, component existence, type compatibility, exclusive physical occupancy, capacity, and unambiguous explicit order through exact reconciliation. Permanent save records still retain only completed-run owned item ids, so routing remains run-local build state.
