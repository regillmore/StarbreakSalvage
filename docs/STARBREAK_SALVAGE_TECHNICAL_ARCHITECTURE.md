...
- `getActiveSectorHazards` exposes `worldProgress` only when the pause-safe effective hazard distance diverges from actual scroll distance. Distant source/target positions translate with actual sector scroll and therefore stay stationary during a scroll hold; the Liang-Barsky arena projection returns only current entrance/exit intersections on the padded combat bounds. Bolt head/tail travel continues from effective phase progress so work order 112 pause-safe expiry remains intact.
- `SectorHazards` uses only the arena-clipped finite moving bolt segment for player, enemy, boss, and ally circles. At most 24 continuous expanded bolt boxes reuse existing environment/set-piece hazard damage paths; offscreen portions produce no sample, and all visible samples pierce rather than terminate on contact.
