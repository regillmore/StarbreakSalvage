...
### Work orders 132, 145, 159, 160, 169, 170, 177, 179, 180, 181, 183, 185, 188, 189, 190, and 214 item circuit boundary
...
- Work order 214 moves Relic Ash Compass into `RunUpgradeEffects.relicAshCompass`. `SectorRewards` gives a restored fitted Compass precedence and otherwise appends the same relic/phase tags only when the resolved reward pool is Vault. The flag is excluded from the expedition-wide fingerprint and enters only the existing named reward roll. Ashwake Reliquary reads at most three upstream phase projectiles in the ordered `onFire` pipeline and appends bounded offset echoes; combat and Foundry preview consume that one production result without another state field, counter, scan, target, or RNG path.
