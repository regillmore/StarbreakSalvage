...
- Work order 138 composes the existing post-sector branch and extraction route inputs in one `SectorTransitionScene`. `GameApp.showMissionBranch` separates branch commitment from its presentation handoff: the optional branch enters combat unchanged, while an onward route commits the default branch, completes its relief reducer stage, and calls the established route handler synchronously. This removes an intermediate scene without bypassing faction/expedition recording or route outcome, reward, service, and sector-advance ordering. The combined four-action board is presentation-only; extraction snapshots and post-optional recovery still reconstruct the ordinary route-only mode.
