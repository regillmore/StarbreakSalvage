...
- Telegraphs clearly distinguish arena entrance from exit, expose direction/offset copy without color alone, and remain anchored to actual sector scroll rather than the viewport or pause-safe hazard clock. Physical source/target positions stay distant and offscreen; their visible intersections remain clipped to and slide along arena edges.
...
Status: implemented. `BeamHazard` defines twelve directional route families, deterministic 14-86% offsets, distant world endpoints extended 22% beyond the arena span, arena-rectangle clipping, a fixed 42%-arena-span bolt body, finite source-to-target travel, exact circle/capsule overlap, direction copy, and at most 24 continuous world-damage boxes. The physical vector translates with actual sector scroll while its visible source and target are recomputed as arena-edge intersections; a top or bottom endpoint therefore slides along that boundary instead of entering the playfield. `ActiveSectorHazard.worldProgress` records actual scroll progress only when the pause-safe hazard clock diverges, so the vector remains fixed during a scroll hold while bolt travel and expiry continue. `CanvasRenderer` paints only the clipped dashed telegraph, entrance aperture, exit reticle, long moving colored shell, white core, and leading flare. `SectorHazards` and world/set-piece damage consume only the clipped visible bolt subsegment; offscreen vector portions never render or collide. Every intersecting player, enemy, boss, ally, and eligible world target retains allegiance-neutral piercing and cooldown behavior. HUD copy reports tracking or bolt-travel percentage. No runtime actor/projectile entity, RNG call, generation fingerprint, save field, or snapshot migration is introduced.

Verification: `npm run verify:release` passes with 93 Vitest files and 561 tests, ESLint, typecheck, production build, all 13 Chromium paths, and the Pages-base production-preview asset smoke. Focused geometry/behavior/feature/runtime/director coverage passes with 41 tests. Coverage pins deterministic directional variety, distant endpoint extension, arena-edge clipping, the top-spawned endpoint regression, marker sliding under actual world scroll, track stability while scrolling is paused, finite bolt growth/traverse/clear, clipped visible-body circle parity, bounded continuous world samples, invalid endpoint rejection, final-plan geometry, active player/enemy/boss/ally damage, and cooldown suppression. The high-contrast reduced-motion environmental-stress Chromium journey exercises the moving beam renderer without a browser error. The production build emits 857.62 kB minified/232.58 kB gzip initial JavaScript and unchanged 31.96 kB CSS/7.19 kB gzip, a 2.15 kB minified/0.69 kB gzip increase over the initially deployed work order 120 beam. The existing chunk warning remains open and no threshold changed. Direct visual inspection was attempted through the in-app browser control surface, but no browser target was available in this session; automated Chromium is the available local visual evidence.
