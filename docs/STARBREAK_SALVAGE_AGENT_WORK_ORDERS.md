...
- Operation Airlock retains objective, conditions, pacing, waves, faction/rival, crew, fleet, apex, and operational-map intelligence while making launch the clear primary action.
...
Operation Airlock retains the previous mission intelligence as one destination instead of presenting every system as a flat page. Shop, Hardpoint Control, Pocket Hangar, Crew Commons, and Signal Vault stay visible and available in ordinary sectors; empty cargo, insufficient tender, absent craft, or an empty crew roster becomes useful detail instead of a hidden menu. Route-shop stops still provide their authored market advantage, while every hub now supplies ordinary carrier market access.
...
Verification: `npm run verify:release` passes with 97 Vitest files and 596 tests, ESLint, typecheck, production build, all 13 Playwright Chromium paths, and the Pages-base production-preview asset smoke. Focused navigation/snapshot/sector-loop coverage passes with 3 files and 19 tests; generation sweeps 64 seeds across all four layouts, proves bounded unique connected nodes, explicit-only story locking, visit/reset behavior, and snapshot v11 round-trip/rejection. The main Chromium journey confirms common service availability, reward-to-hub transit, recovered-hardware detail, optional Hardpoint entry, coherent return, persisted Visited state, launch, and no console errors. The build emits 910.81 kB minified/247.77 kB gzip initial JavaScript and 59.77/12.52 kB CSS. The existing initial-chunk warning remains open and no threshold changed. The in-app browser exposed no available target, so the repository Chromium/production-preview passes provide local visual and runtime evidence; direct deployment inspection remains the final visual check.
