...
## Work order 134 - Act constellations and integrated approaches

Goal: turn the carrier map into a persistent act-scale sector constellation, free common services from route connectivity, and replace the separate approach-choice menu with newly resolved nodes on the shared chart.
...
- The old branch choice-card grid is removed. Direct and optional mission approaches appear as temporary connected constellation nodes, selection updates a compact detail pane, and a separate commit action dispatches the unchanged mission branch event.
...
Status: implemented. `ActConstellation` owns four bounded five-node layouts, the act-specific seed stream, stable chart identity, completed/current/revealed/hidden projection, sequential edge states, and temporary approach-node placement. `SectorNavigation` now composes that chart with five shuffled peripheral service positions; Operation Airlock is the current sector itself, while services remain available but never enter the edge list. The chart is stable across all five sectors of an act and regenerated from the next act id at handoff.
...
`OperationalMapScene` retains its non-choice relief itinerary but replaces the approach choice-card menu with the current act chart plus temporary direct/optional vector nodes. Selecting a vector updates its narrative, exposure, and target detail; `Commit` dispatches the existing mission branch event, after which faction/rival/crew/carrier records and combat/relief/extraction flow continue unchanged. Existing operational-map checkpoint targets still restore into the new chart, so no snapshot migration or new stored state is required.

Verification: `npm run verify:release` passes with 97 Vitest files and 598 tests, ESLint, typecheck, production build, all 13 Playwright Chromium paths, and the Pages-base production-preview asset smoke. Six focused navigation tests cover act-stable/act-refreshed identities, exact reveal promotion, four-layout seed sweeps, sector-only connectivity, temporary approach edges, story locks, visits, and snapshot v11. Chromium asserts five sector nodes, five floating services, four service-free edges, one revealed-next node, inspect-then-commit optional/direct approaches, settled operational-map reload/resume, narrow/high-contrast/reduced-motion paths, and the full reward-to-next-sector loop without console errors. The build emits 919.63 kB minified/250.40 kB gzip initial JavaScript and 62.12/12.89 kB CSS, increases of 9.92/2.98 kB JavaScript and 2.35/0.37 kB CSS over the refined WO133 result. No simulation hot path, actor pool, RNG contract, dependency, or size threshold changes; the existing initial-chunk warning remains open. The Browser skill catalog still points to an evicted cache version, so Playwright Chromium supplies local visual/runtime evidence and direct deployment inspection remains the final visual check.
