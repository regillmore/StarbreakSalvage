...
- Telegraphs clearly distinguish the emitting endpoint from the terminal reticle, expose direction/offset copy without color alone, and remain anchored to actual sector scroll rather than the viewport or pause-safe hazard clock.
- The active beam grows from its source, traverses the track at finite velocity, and clears its destination with readable leading and trailing edges; the exact visible bolt segment drives player/enemy/boss/ally collision.
...
Status: implemented. `BeamHazard` defines twelve edge-to-edge route families, deterministic 14-86% endpoint offsets, a world-anchored track projection, a fixed 42%-track bolt body, finite source-to-target travel, exact circle/capsule overlap, direction copy, and at most 24 continuous world-damage boxes. `ActiveSectorHazard.worldProgress` records actual scroll progress only when the pause-safe hazard clock diverges, so the track remains fixed during a scroll hold while bolt travel and expiry continue. `HazardZoneDirector` still attaches route geometry after the operation-specific sequence is chosen and includes endpoints in deterministic summaries. `CanvasRenderer` paints the full track as a dashed telegraph, keeps its source aperture and terminal reticle world-anchored, and during firing replaces the instant full-field body with a long moving colored shell, white core, and leading flare. `SectorHazards` and world/set-piece damage now consume only that visible moving subsegment; every intersecting player, enemy, boss, ally, and eligible world target retains allegiance-neutral piercing and cooldown behavior. HUD copy reports tracking or bolt-travel percentage. No runtime actor/projectile entity, RNG call, generation fingerprint, save field, or snapshot migration is introduced.

Verification: `npm run verify:release` passes with 93 Vitest files and 561 tests, ESLint, typecheck, production build, all 13 Chromium paths, and the Pages-base production-preview asset smoke. Focused geometry/behavior/feature/director coverage passes with 36 tests; a second geometry/behavior/feature/runtime pass covers 35 tests. Coverage pins deterministic directional variety, distinct edges, world-track translation, track stability while actual scrolling is paused, finite bolt growth/traverse/clear, visible-body circle parity, bounded continuous world samples, invalid endpoint rejection, final-plan geometry, active player/enemy/boss/ally damage, and cooldown suppression. The high-contrast reduced-motion environmental-stress Chromium journey exercises the moving beam renderer without a browser error. The production build emits 856.92 kB minified/232.28 kB gzip initial JavaScript and unchanged 31.96 kB CSS/7.19 kB gzip, a 1.45 kB minified/0.39 kB gzip increase over the initially deployed work order 120 beam. The existing chunk warning remains open and no threshold changed. Direct visual inspection was attempted through the in-app browser control surface, but no browser target was available in this session; automated Chromium is the available local visual evidence.
