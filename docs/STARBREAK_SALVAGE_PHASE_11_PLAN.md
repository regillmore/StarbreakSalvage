...
Status: implemented by work order 102, refitted to the constellation by work order 139, reward-reordered by work order 140, given a direct Act I refit boundary by work order 141, made safely suspendable by work order 142, expanded into a `1-2-3-2-1` graph by work order 143, and given destination-owned route effects by work order 146. Expedition graph schema v2 still generates stable compatibility nodes per sector, but fresh ordinary mission schedules enter the full required gate operation directly, settle its reward, then offer one paired pursuit beside the legal next destinations. Each destination now binds one seeded base effect from its own candidate pool, with easier/harder graph ranks biasing safer/severe effects; there is no secondary route trio. At Act I convergence, the default branch, relief, and extraction settle automatically into the midpoint refit instead of projecting an Act II route on the completed chart. Advance, detour, staging, and the former approach branch remain only for checkpoint recovery and Scenario Lab fixtures. Successful reachable pursuits retain their one-shot salvage, shape the following sector's reward and pressure, and never issue a duplicate reward scene. Every live constellation mode checkpoints before an explicit main-menu exit and resumes the same briefing, optional/destination board, or unresolved route plot through existing snapshot targets. `OperationalMap` keeps the bounded idempotent settlement ledger and compatibility read model while fresh choices live in the constellation. Current fifteen-sector authored graph projection is 1,009 seconds (16.82 minutes) standard and 1,264 seconds (21.07 minutes) with all authored holds; these remain structural estimates awaiting deployed full-run timing.
...
Status: implemented by work order 103 and topology-refined by work order 134. Three seed/save-fingerprint-stable campaign variants order five original frontier families and their environmental laws while preserving Horizon Scar as the final anchor. Act III uses the existing expedition graph, mission director, sector conditions, backgrounds, hazards, route/economy, engineering, faction, boss arena, save, summary, timeline, and accessibility contracts. The Act II boundary now offers a complete ten-sector extraction victory or an idempotent state-carrying breach; snapshot v3 persists the choice and safely retires v1/v2. Standard graph capacity is 1,689 seconds (28.15 minutes), with 1,944 seconds (32.40 minutes) when the one executable optional hold paired with every sector is taken.
