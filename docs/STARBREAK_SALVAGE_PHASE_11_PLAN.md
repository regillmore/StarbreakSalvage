...
Status: implemented by work order 102, refitted to the constellation by work order 139, reward-reordered by work order 140, given a direct Act I refit boundary by work order 141, and made safely suspendable by work order 142. Expedition graph schema v2 still generates seven stable compatibility nodes per sector, but fresh ordinary mission schedules enter the full required gate operation directly, settle its reward, then offer one paired pursuit beside the next route. At Act I sector 5, the default branch, relief, and extraction settle automatically into the midpoint refit instead of projecting an Act II route on the completed chart. Advance, detour, staging, and the former approach branch remain only for deployed v11 recovery and Scenario Lab fixtures. Successful reachable pursuits retain their one-shot salvage, shape the following sector's reward and pressure, and never issue a duplicate reward scene. Every live constellation mode now checkpoints before an explicit main-menu exit and resumes the same briefing, optional/route board, or unresolved three-route plot through existing v11 targets. `OperationalMap` keeps the bounded idempotent settlement ledger and compatibility read model while fresh choices live in the constellation. Current fifteen-sector authored graph projection is 1,009 seconds (16.82 minutes) standard and 1,264 seconds (21.07 minutes) with all authored holds; these remain structural estimates awaiting deployed full-run timing.
