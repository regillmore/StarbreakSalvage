...
## Work order 225 - Shared thermal circuit

Goal: turn heat from a mostly punitive weapon meter into an ordered build resource with deliberate sources, sinks, thresholds, payoffs, and legible feedback.

Prompt:

> Review and refresh heat sources, heat sinks, synergy, and feedback loops so heat-focused circuits can build, preserve, spend, and profit from weapon heat instead of only avoiding overheat.

Acceptance criteria:

- Establish one shared cold, working, hot, critical, and overheated read model for combat, item hooks, and Hardpoint simulation.
- Make Plasma Seed Crucible an explicit per-volley source while retaining its upstream-trait plasma conversion.
- Rework Overheat Oracle into a periodic thermostat that generates heat while cool and spends it for a stronger plasma omen while hot.
- Keep Heat Sink Saint as the passive heat-efficiency anchor and add an active hot-band sink that stabilizes stronger, longer-lived shots.
- Scale Heat Signature Loop's heat/plasma impact, size, and reach with current stored heat so carrying heat creates an offensive payoff.
- Let Prototype Vent Script and other later sinks consume heat generated by earlier circuit stages in the same volley.
- Show generated and spent heat in the eight-second Foundry scope without redefining Base DPS, and label hot volleys explicitly.
- Render distinct bounded intake and sink cues in combat, add HOT/CRITICAL HUD warning treatment, and expose the current thermal band in the weapon readout.
- Preserve item count, rarity, reward pools, circuit ordering, deterministic content, saves, dependencies, actor caps, and static hosting.

Status: implemented. `ThermalCircuit` now owns the shared bands and authored source/sink ratios. Ordered `onFire` payloads carry bounded thermal-flow events, so an earlier Plasma Seed can fund a later periodic Vent in the same volley. Plasma Seed, Overheat Oracle, Heat Sink Saint, and Heat Signature Loop now form a source/thermostat/sink/payoff loop around the existing large Vent spender.

`CombatState` resolves the shared flow before ordinary shot heat and emits inward amber intake or outward cyan sink effects. The HUD advances through shared `COLD`, `WORKING`, `HOT`, `CRITICAL`, and `OVERHEAT` labels with an amber hot-band meter warning. Hardpoint Control uses the same reducer inputs, reports generated/spent circuit flow and hot-volley count, and retains the independent direct-damage measurement boundary.
